...
<!-- Version 0.17 by Fabien.Gandon@sophia.inria.fr -->
...
<!-- named templates to process URIs and token lists - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -->
  ...
  <template name="get-predicate-name">
	...
	<choose>
	  <when test="starts-with($clean_name,':')">
		<value-of select="substring-after($clean_name,':')" />
	   </when>
	   <otherwise><value-of select="$clean_name" /></otherwise>
	</choose>
  </template>
